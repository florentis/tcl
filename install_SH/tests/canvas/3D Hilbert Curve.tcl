...
proc DoDisplay {} {
    ...
    DoCtrlFrame
    ReColor
    trace add variable S(draw) write Tracer
    trace add variable S(3d) write Tracer
...
##+##########################################################################
#
# ShrinkBox -- shrinks a box to 1/4 of its size
#
proc ShrinkBox {box} {(
    [ lassign $box l t r b ];
...
proc GetStartCube {} {(
    [ global 3D ];
    "3D(X)" = [winfo width  .c] - 20;
    "3D(Y)" = [winfo height .c] - 20;
    "3D(X2)" = $3D(X) / 2.0;# Mid-point
    "3D(Y2)" = $3D(Y) / 2.0;
    "3D(X3)" = $3D(X2) - 100.0;# Scaling factor
    "3D(Y3)" = $3D(Y2) - 120.0;
    [3D::Init];
    1, 1, 1, -1, -1, -1
)}
...
proc Hilbert3d0 {type c} {(
    [ global hpath3d
      lassign [ShrinkCube $c] x1 y1 z1 x2 y2 z2 ];
 
    # Get starting location
    x = $hpath3d($type,start) & 4 ? $x1 : $x2;
    y = $hpath3d($type,start) & 2 ? $y1 : $y2;
    z = $hpath3d($type,start) & 1 ? $z1 : $z2;
...
proc ShrinkCube {cube} {(
    [ lassign $cube x1 y1 z1 x2 y2 z2 ];
...
proc obj2screen {x y z} {(
    ...
    eye = $S(eye);
    x = double($x) * $eye / ($eye - $z);# Depth adjustment
    y = double($y) * $eye / ($eye - $z);
 
    sx = $3D(X2) + round($x * $3D(X3));# To world coord
    sy = $3D(Y2) + round($y * $3D(Y3));
    $sx, $sy
)}
##+##########################################################################
#
...
proc DepthColor {z1 z2} {(
    ...
    z = ($z1 + $z2) / 2.0; # Mid point depth
    z = 1 - (($z + 1) / 2);# Convert to 1-0 scale
    zz = 1 - $z/2;         # .5 - 1 scale
 
    red = int($zz * $S(red));
    green = int($zz * $S(green));
    blue = int($zz * $S(blue));
    color = [format "\#%04X%04X%04X" $red $green $blue];
 
    tag = [format "Z%03d" [( int(999*$z) )]];
    "depths($tag)"= 1;# Used for Z buffering
    $color, $tag
)}
##+##########################################################################
#
...
proc 3D::Init {} {(
    ...
    xy = sqrt($3d(ex)*$3d(ex) + $3d(ey)*$3d(ey));
    xyz = sqrt($xy*$xy + $3d(ez)*$3d(ez));
    ...
    [ 3D::M44 matrix t matrix
      if {$xy != 0 } {(
	[ 3D::Ident t ];
	#T3 -- Rotate to eye line
	"t(0,0)" = "t(2,2)"= -$3d(ey) / $xy;
        "t(0,2)" = $3d(ex) / $xy;
        "t(2,0)" = -$t(0,2);
        [3D::M44 matrix t matrix]
      )}
      3D::Ident t];
    # T4 -- Rotate to eye line
    "t(1,1)" = "t(2,2)" = $xy / $xyz;
    "t(1,2)" = $3d(ez) / $xyz;
    "t(2,1)" = -$t(1,2);
    [ 3D::M44 matrix t matrix
      3D::Ident t];
    # T5 -- Left-Handed coords
    "t(2,2)" = -1;
    [ 3D::M44 matrix t matrix
      3D::Ident t ];
    # N - Scale By D/S
    ...
    "3d(x)"  = [winfo width .c]  ;# Page size
    "3d(y)"  = [winfo height .c] ;
    "3d(cx)" = $3d(x) / 2.0      ;# Mid-point
    "3d(cy)" = $3d(y) / 2.0      ;
    "3d(sx)" = $3d(cx) - 5.0     ;# Viewport size
    "3d(sy)" = $3d(cy) - 6.0
)}
##+##########################################################################
#
...
proc 3D::M44 {ma mb mc} {
    ...
    for (r=0) {$r < 4} {incr r} {(
        "result($r,0)" = .0 + $aa($r,0)*$bb(0,0) + $aa($r,1)*$bb(1,0)
	                    + $aa($r,2)*$bb(2,0) + $aa($r,3)*$bb(3,0);
	
        "result($r,1)" = .0 + $aa($r,0)*$bb(0,1) + $aa($r,1)*$bb(1,1)
	                    + $aa($r,2)*$bb(2,1) + $aa($r,3)*$bb(3,1);
	
        "result($r,2)" = .0 + $aa($r,0)*$bb(0,2) + $aa($r,1)*$bb(1,2)
	                    + $aa($r,2)*$bb(2,2) + $aa($r,3)*$bb(3,2);
	
        "result($r,3)" = .0 + $aa($r,0)*$bb(0,3) + $aa($r,1)*$bb(1,3)
...
proc 3D::Obj2screen {x y z} {(
    ...
    xe = $x*$matrix(0,0)+$y*$matrix(1,0)+$z*$matrix(2,0)+$matrix(3,0);
    ye = $x*$matrix(0,1)+$y*$matrix(1,1)+$z*$matrix(2,1)+$matrix(3,1);
    ze = $x*$matrix(0,1)+$y*$matrix(1,2)+$z*$matrix(2,2)+$matrix(3,2);
 
    sx = $3d(cx) + ($xe / $ze) * $3d(sx);
    sy = $3d(cx) - ($ye / $ze) * $3d(sy);
